const fetch = require('node-fetch');

module.exports = async (req, res) => {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET, POST, OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  if (req.method === 'OPTIONS') return res.status(200).end();

  const { query, contact, with_related } = req.query || {};
  const body = req.body || {};
  const { action, order, order_id, return_items, reason } = body;

  const token = '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';
  const storeDomain = 'trueweststore.myshopify.com';

  // ==================== EXCHANGE: CREATE REPLACEMENT ORDER ====================
  if (req.method === 'POST' && action === 'submit_exchange') {
    try {
      const originalName = order.name || 'unknown';
      const newOrderRes = await fetch(`https://${storeDomain}/admin/api/2024-07/orders.json`, {
        method: 'POST',
        headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          order: {
            ...order,
            note: `Exchange for ${originalName} via portal`,
            financial_status: "paid",
            tags: "exchange-order,portal-created"
          }
        })
      });

      if (!newOrderRes.ok) throw new Error(await newOrderRes.text());
      const newOrder = (await newOrderRes.json()).order;

      // Tag original as processed
      const origRes = await fetch(`https://${storeDomain}/admin/api/2024-07/orders.json?name=${originalName}`, {
        headers: { 'X-Shopify-Access-Token': token }
      });
      const origData = await origRes.json();
      if (origData.orders?.[0]) {
        await fetch(`https://${storeDomain}/admin/api/2024-07/orders/${origData.orders[0].id}.json`, {
          method: 'PUT',
          headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
          body: JSON.stringify({ order: { tags: "exchange-processed,portal-exchange" } })
        });
      }

      res.json({ success: true, exchange_order: newOrder });
    } catch (e) {
      res.status(500).json({ success: false, error: e.message });
    }
    return;
  }

  // ==================== RETURN: REAL SHOPIFY RETURN ====================
  if (req.method === 'POST' && action === 'submit_return') {
    try {
      const returnRes = await fetch(`https://${storeDomain}/admin/api/2024-07/returns.json`, {
        method: 'POST',
        headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          return: {
            order_id: parseInt(order_id),
            notify_customer: true,
            note: `Portal return - ${reason}`,
            refund: true,
            return_line_items: return_items.map(i => ({
              line_item_id: i.line_item_id,
              quantity: i.quantity || 1,
              restock_type: "return"
            }))
          }
        })
      });

      if (!returnRes.ok) throw new Error(await returnRes.text());
      await returnRes.json();

      await fetch(`https://${storeDomain}/admin/api/2024-07/orders/${order_id}.json`, {
        method: 'PUT',
        headers: { 'X-Shopify-Access-Token': token, 'Content-Type': 'application/json' },
        body: JSON.stringify({ order: { tags: "return-processed,portal-return" } })
      });

      const amount = return_items.reduce((s, i) => s + (i.price || 0) * (i.quantity || 1), 0).toFixed(2);
      res.json({ success: true, refund_amount: amount });
    } catch (e) {
      res.status(500).json({ success: false, error: e.message });
    }
    return;
  }

  // ==================== GET ORDER - YOUR ORIGINAL WORKING LOGIC + FIXED DETECTION ====================
  if (req.method === 'GET' && query && contact) {
    try {
      const cleanQuery = query.replace('#', '').trim();

      // Find customer
      const field = contact.includes('@') ? 'email' : 'phone';
      const custRes = await fetch(`https://${storeDomain}/admin/api/2024-07/customers/search.json?query=${field}:${encodeURIComponent(contact)}`, {
        headers: { 'X-Shopify-Access-Token': token }
      });
      const custData = await custRes.json();
      if (!custData.customers?.length) return res.status(404).json({ error: 'Customer not found' });
      const customerId = custData.customers[0].id;

      // Get all customer orders
      const allRes = await fetch(`https://${storeDomain}/admin/api/2024-07/orders.json?customer_id=${customerId}&status=any&limit=250`, {
        headers: { 'X-Shopify-Access-Token': token }
      });
      const allOrders = (await allRes.json()).orders || [];

      const mainOrder = allOrders.find(o => o.name.includes(cleanQuery) || String(o.order_number) === cleanQuery);
      if (!mainOrder) return res.status(404).json({ error: 'Order not found' });

      await enhanceOrder(mainOrder);

      const response = {
        orders: [mainOrder],
        already_processed: false,
        exchange_order_name: null,
        related_order: null
      };

      const tags = (mainOrder.tags || '').toLowerCase();
      const note = (mainOrder.note || '').toLowerCase();

      // ORIGINAL order already exchanged
      if (tags.includes('exchange-processed') || tags.includes('return-processed')) {
        response.already_processed = true;
        const replacement = allOrders.find(o => o.note && o.note.toLowerCase().includes(mainOrder.name.toLowerCase()));
        if (replacement) {
          response.exchange_order_name = replacement.name;
          await enhanceOrder(replacement);
          response.related_order = replacement;
        }
      }
      // This IS the replacement order
      else if (note.includes('exchange for') || note.includes('portal')) {
        const match = note.match(/#(\d+)/i);
        if (match) {
          const original = allOrders.find(o => o.name.includes(match[1]));
          if (original) {
            await enhanceOrder(original);
            response.related_order = original;
          }
        }
      }

      res.json(response);
    } catch (e) {
      console.error(e);
      res.status(500).json({ error: 'Server error' });
    }
    return;
  }

  res.status(400).json({ error: 'Invalid' });
};

// YOUR ORIGINAL WORKING ENHANCE FUNCTION — UNTOUCHED, PERFECT
async function enhanceOrder(order) {
  order.line_items = order.line_items || [];
  let deliveredAt = null;
  let status = 'Processing';

  if (order.fulfillments?.[0]?.tracking_number) {
    const awb = order.fulfillments[0].tracking_number.trim();
    try {
      const r = await fetch(`https://track.eshipz.com/track?awb=${awb}`, { timeout: 10000 });
      const html = await r.text();

      if (html.includes('Delivered')) {
        const m = html.match(/(\d{2}\/\d{2}\/\d{4})/);
        if (m) deliveredAt = m[1];
        status = 'Delivered';
      } else if (html.includes('In Transit') || html.includes('Out for Delivery')) status = 'In Transit';
      else if (html.includes('Picked Up')) status = 'Picked Up';
    } catch (e) { /* silent */ }
  }

  order.delivered_at = deliveredAt;
  order.current_shipping_status = status;

  for (let item of order.line_items) {
    try {
      const p = await fetch(`https://${storeDomain}/admin/api/2024-07/products/${item.product_id}.json?fields=images,variants`, {
        headers: { 'X-Shopify-Access-Token': token }
      });
      const prod = (await p.json()).product;
      item.image_url = prod.images?.[0]?.src || 'https://via.placeholder.com/80';
      const v = prod.variants.find(v => v.id === item.variant_id);
      item.current_size = v?.title || 'Standard';
      item.available_variants = (prod.variants || []).filter(v => v.inventory_quantity > 0).map(v => ({ id: v.id, title: v.title }));
    } catch (e) {
      item.image_url = 'https://via.placeholder.com/80';
      item.current_size = 'Standard';
    }
  }
}
